<?php
/**
 * cetak-label-a4-mix-right.php — Label Mixcode Warna, barcode di KANAN.
 *
 * Port layout "rightCode_template.php" dari plugin SLiMS
 * label_mixcode_color_slims ke Inlislite (TCPDF/writeHTML + ekspor Word).
 *
 * Tiap label: [no. panggil | barcode vertikal], header nama
 * perpustakaan dengan latar warna kelas (dari master_kelas_besar
 * via $label['Warna1'], setara lbc_color 0XX..9XX di SLiMS).
 *
...
foreach ($LabelData as $label) {
    ...
    $html .= '<td style="width:' . $mixColW . ';">'
        ...
        . '</tr>'
        . '<tr>'
        . '<td style="width:65%; border-bottom:solid 1px #CCC; border-left:solid 1px #CCC; border-right:solid 1px #CCC; text-align: center; vertical-align: middle; padding: 4px 3px;">'
        . '<span style="font-size: 11px; font-weight: bold;">' . $callLines . '</span>'
        . '</td>'
        . '<td rowspan="2" style="width:35%; text-align: center; vertical-align: middle; padding: 4px 3px; border-bottom:solid 1px #CCC; border-right:solid 1px #CCC;">'
        . ($barcodeImg !== '' ? '<img src="' . $barcodeImg . '" width="32" height="' . $mixBarcodeHeight . '">' : $barcodeTxt)
        . '</td>'
        . '</tr>'
        . '<tr>'
        . '<td style="border-bottom:solid 1px #CCC; border-left:solid 1px #CCC; border-right:solid 1px #CCC; border-top:solid 1px #CCC; text-align: center; vertical-align: middle; padding: 4px 3px;">'
...
$pdf->Output('label-mixcode-kanan.pdf', 'D');
die;
