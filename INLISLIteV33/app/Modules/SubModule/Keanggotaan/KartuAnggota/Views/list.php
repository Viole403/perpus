...
<div class="app-main__inner">
		...
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>KartuAnggota <?= ucwords(unslugify($slug)) ?>
					<div class="page-title-subheading">Daftar semua KartuAnggota
					</div>
				</div>
			</div>
	...
	<ul class="body-tabs body-tabs-layout tabs-animated body-tabs-animated nav">
		<?php foreach (get_ref('ref-kartuanggota', 'slug') as $row) : ?>
			<li class="nav-item">
				<a class="nav-link <?= ($row->slug == $slug) ? 'active' : '' ?>" href="<?= base_url('master/kartuanggota?slug=' . $row->slug) ?>">
	...
	<div class="main-card mb-3 card">
		<div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate">
			</i>Tabel KartuAnggota <?= ucwords(unslugify($slug)) ?>
			<div class="btn-actions-pane-right actions-icon-btn">
				<?php if (is_allowed('kartuanggota/create')) : ?>
					<a href="<?= base_url(
									'master/kartuanggota/create?slug=' . $slug
								) ?>" class=" btn btn-success" title=""><i class="fa fa-plus"></i>
						Tambah KartuAnggota </a>
				<?php endif; ?>
			</div>
		</div>
		<div class="card-body">
