    ...
    public function datatable($slug = null)
    {
        ...
        $dataTable = DataTable::of($builder)
            ...
            ->edit('active', function ($row) {
                ...
                $html = '<input type="checkbox" class="apply-status" data-href="' . base_url('api/master-template/switch/' . $row->id) . '" data-checked="' . $checked . '" data-field="active" ' . $checked . ' data-toggle="toggle" data-onstyle="success" data-on="Ya" data-off="Tdk" data-size="mini">';
                return $html;
            })
            ...
            ->edit('action', function ($row) {
                $upload = '<a href="javascript:void(0);" 
					...
					data-url="' . base_url('api/master-template/upload_file') . '"  
					data-dz_url="' . base_url('master-template/do_upload') . '" 
					data-controller="template" 
					data-title="Upload File"
					data-sub_title="Format (.jpg, .jpeg, .png). Max 5MB"
					data-redirect="' . base_url('master-template') . '" 
					data-format=".jpg,.jpeg,.png" 
					data-count="1" 
					data-max="10" 
					data-toggle="tooltip" data-placement="top" title="Upload File" class="btn btn-primary upload-data">
					<i class="fa fa-upload"> </i> </a>';
                $edit = '<a href="' . base_url('master-template/edit/' . $row->id) . '" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-warning show-data"><i class="pe-7s-note font-weight-bold"> </i></a>';
                $delete = '<a href="javascript:void(0);" data-href="' . base_url('master-template/delete/' . $row->id) . '" data-toggle="tooltip" data-placement="top" title="Hapus " class="btn btn-danger remove-data"><i class="pe-7s-trash font-weight-bold"> </i></a>';
