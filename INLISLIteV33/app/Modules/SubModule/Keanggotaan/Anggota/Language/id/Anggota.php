<?php

return [
   'module' => 'Anggota',
   'module_keranjang' => 'Keranjang Anggota',

   'form.complete_the_data' => 'Mohon lengkapi data pada form berikut',

   'field.no' => 'No',
   'field.status' => 'Status',
   'field.id' => 'ID',
   'field.name' => 'Nama',
   'field.photo' => 'Foto',
   'field.MemberNo' => 'Nomor Anggota',
   'field.Email' => 'Email',
   'field.Jenisidentitas'=>'Jenis Identitas',
   'field.Jenisanggota'=>'Jenis Anggota',
   'field.Statusperkawinan'=>'Status Perkawinan',
   'field.Agama'=>'Agama',
   'field.nomoridentitas'=>'Nomor Identitas',
   'field.Pendidikan'=>'Pendidikan',
   'field.Jeniskelamin'=>'Jenis Kelamin',
   'field.Unitkerja'=>'Unit Kerja',
   'field.fakultas'=>'Fakultas',
   'field.Jurusan'=>'Jurusan',
   'field.Tanggalpendaftaran'=>'Tanggal Pendaftaran',
   'field.Masaberlaku'=>'Masa Berlaku',
   'field.Statusanggota'=>'Status Anggota',
   'field.Biayapendaftaran'=>'Biaya Pendaftaran',
   'field.Tempatlahir'=>'Tempat lahir',
   'field.DateOfBirth'=>'Tanggal Lahir',
   'field.provinsi'=>'Provinsi',
   'field.library'=>'Lokasi Perpustakaan',
   'field.city'=>'Kab/Kota',
   'field.peminjaman'=>'Nomor peminjaman',
   'field.pelanggaran'=>'Data-Pelanggaran',
   'field.peminjaman'=>'Data-Peminjaman',
   'field.perpanjangan'=>'Data-Perpanjangan',
   'field.sumbangan'=>'Data-sumbangan',

   'field.slug' => 'Slug',
   'field.sort' => 'Urutan',
   'field.category' => 'Kategori',
   'field.description' => 'Keterangan',
   'field.active' => 'Aktif',
   'field.iskeranjang' => 'Keranjang Anggota',
   'field.created_by' => 'Dibuat oleh',
   'field.updated_by' => 'Diubah oleh',
   'field.created_at' => 'Tgl dibuat',
   'field.updated_at' => 'Tgl diubah',
   'field.deleted_at' => 'Tgl dihapus',

   'label.table' => 'Tabel',
   'label.action' => 'Aksi',
   'label.home' => 'Beranda',
   'label.active' => 'Aktif',
   'label.inactive' => 'Nonaktif',
   'label.search' => 'Cari',

   'info.list_all' => 'Daftar Semua',
   'info.not_found' => 'Tidak ditemukan',
   'info.success' => 'Berhasil',
   'info.successfully_saved' => 'Anggota berhasil disimpan',
   'info.successfully_deleted' => 'Anggota berhasil dihapus',
   'info.successfully_updated' => 'Anggota berhasil diubah',
   'info.error' => 'Error',
   'info.failed' => 'Gagal',
   'info.failed_to_save' => 'Anggota gagal disimpan',
   'info.failed_to_delete' => 'Anggota gagal dihapus',
   'info.failed_to_update' => 'Anggota gagal diubah',
   'info.warning' => 'Peringatan',
   'info.are_you_sure' => 'Anda yakin ?',
   'info.can_not_be_restored' => 'Tidak bisa dikembalikan',

   'action.list'  => 'Daftar',
   'action.all'  => 'Semua',
   'action.add'  => 'Entri',
   'action.new'  => 'Baru',
   'action.save'  => 'Simpan',
   'action.edit'  => 'Edit',
   'action.update'  => 'Ubah',
   'action.pelanggaran'  => 'Data-pelanggaran',
   'action.view'  => 'Lihat',
   'action.detail'  => 'Detil',
   'action.delete'  => 'Hapus',
   'action.activate'  => 'Aktifkan',
   'action.deactivate'  => 'Nonaktifkan',
];
