...
<div class="app-main__inner">
		...
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>Pengaturananggota <?= ucwords(unslugify($slug)) ?>
					<div class="page-title-subheading">Daftar semua Pengaturananggota
					</div>
				</div>
			</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-header"><i class="header-icon lnr-list icon-gradient bg-plum-plate">
			</i>Tabel Pengaturananggota <?= ucwords(unslugify($slug)) ?>
			<div class="btn-actions-pane-right actions-icon-btn">
				<?php if (is_allowed('pengaturananggota/create')) : ?>
					<a href="<?= base_url('pengaturananggota/create?slug=' . $slug) ?>" class=" btn btn-success" title=""><i class="fa fa-plus"></i>
						Tambah Pengaturananggota </a>
				<?php endif; ?>
			</div>
		</div>
		<div class="card-body">
...
<?= $this->section('script'); ?>
<?= $this->include('Pengaturananggota\Views\upload_modal'); ?>
<script>
	var t;
	$(document).ready(function() {
		t = $('#tbl_pages').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": '<?php echo site_url('api/pengaturananggota/datatable/' . $slug) ?>',
			},
