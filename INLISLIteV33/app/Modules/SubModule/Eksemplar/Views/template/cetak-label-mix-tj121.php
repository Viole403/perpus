<?php
/**
 * cetak-label-mix-tj121.php — Label Mixcode untuk Kertas Tom & Jerry 121.
 *
 * Mengikuti pola bawaan cetak-label-tj121-1.php: A4, grid 2 kolom x 5 baris
 * (label 99 x 57 mm, margin atas 18.5mm, kiri 7mm, gap kolom 3mm).
...
// ── Konfigurasi grid (sama seperti bawaan TJ121-1) ──
$cols       = 2;
$rows       = 5;
$labelW     = 99.0;
$labelH     = 57.0;
$marginLeft = 4.5; // (210 - (2x99 + 3)) / 2: grid disenterkan, sisa simetris
$marginTop  = 18.5;
$gapCol     = 3.0;
$gapRow     = 0;
...
foreach ($LabelData as $label) {
    ...
    $barcodeCell = function ($width) use ($barcodeImg, $barcodeTxt) {
        return '<td style="width:' . $width . 'mm; text-align: center; vertical-align: middle;'
            ...
            . ($barcodeImg !== '' ? '<img src="' . $barcodeImg . '" style="width:11mm; height:34mm;">' : $barcodeTxt)
            . '</td>';
    };
    $textCell = '<td style="text-align: center; vertical-align: middle;'
        ...
        . '<span style="font-size:8pt; font-weight:bold;">' . $callLines . '</span>'
        . '<br><span style="font-size:7pt;">' . $titleSlice . '</span>'
        . '</td>';

    if ($position === 'left') {
        $bodyRow = $barcodeCell('24') . $textCell;
        $colspan = 2;
    } elseif ($position === 'both') {
        $bodyRow = $barcodeCell('18') . $textCell . $barcodeCell('18');
        $colspan = 3;
    } else {
        $bodyRow = $textCell . $barcodeCell('24');
        $colspan = 2;
    }
    ...
    <table cellpadding="1" cellspacing="0" style="width:' . $labelW . 'mm;">
        <tr>
            <td colspan="' . $colspan . '" style="border:solid 1px #CCC; height:11mm; text-align: center; vertical-align: middle;
                       font-size:8pt; font-weight:bold; background-color: ' . htmlspecialchars($warna) . ';">'
                . $library .
            '</td>
        </tr>
        <tr style="height:42mm;">' . $bodyRow . '</tr>
    </table>';
...
$pdf->Output('label-mixcode-tj121.pdf', 'D');
die;
